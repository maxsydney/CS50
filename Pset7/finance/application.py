...
def quote():
    ...
    if request.method == "GET":
        render_template("templates/quote.html")
    else:
        if not request.form.get("ticker"):
            return apology("Please enter a stock ticker")
            
        symbol = lookup(request.form.get("ticker"))
...
def register():
    ...
    if request.method == 'POST':
        ...
        result = db.execute("INSERT INTO users (username, hash) VALUES (:name, :hashed)", name=request.form.get("username"), hashed=pwd_context.hash(request.form.get("password")))
